...
class runtime_knobs(base_knobs):
    ...
    # Hook for inspecting compiler pipeline stages. Called twice:
    #   1. With no args during compile()/jit.run() to obtain (key, hash) for
    #      cache key / specialization.
    #   2. With (stages, options, language, capability) in backend
    #      populate_stages() to let the hook rewrite stage callables.
